...
const AddRecipetoCollection = ({ onClose, show, recipe_id }) => {
  const { addToCollection, createCollection, getCollections } = useContext(AuthContext);
  ...
  const saveToCollection = async (collection_id) => {
    ...
    await addToCollection(recipe_id, collection_id);
    onClose(false);
    setIndex(0);
  }

  const saveToNewCollection = async () => {
    const id = await createCollection(nameCollection)
